...
def mostrar_opciones():
    '''
    Función que solo muestra las opciones del menú.
    '''
    print("******************  MENÚ  ******************")
    time.sleep(1)
...
def primera_carga(cursor):
    ...
    Función que genera la primera carga de la base de datos
        Parametros:
            cursor: objeto que permite la conexión con SQL Server
    '''
    flag = False
...
def mostrar_reproducciones(cursor, orden):
    '''
    ...
    if orden == "1":
        ...
        if len(lista_ordenada) == 0:
            print("No haz reproducido canciones anteriormente")
            return        
...
def reproducir_cancion(cursor, nombre_cancion):
    '''
    Función que muestra las canciones que el usuario haya escuchado en los X días
        Parametros:
            cursor: objeto que permite la conexión con SQL Server.
            nombre_cancion(String): Nombre de la canción que se quiere reproducir.
    '''
...
def buscar_en_reproduccion(cursor,nombre):
    '''
    Función que  busca y muestra la cancion y su información de la tabla reproduccion
        Parametros:
            cursor: objeto que permite la conexión con SQL Server
            nombre(String): nombre de la cancion que se quiere buscar.
    '''
...
def mostrar_por_fecha(cursor, delta_fecha, dias):
    '''
    Función que muestra las canciones que el usuario haya escuchado en los X días
        Parametros:
            cursor: objeto que permite la conexión con SQL Server
            delta_fecha(String): fecha por el cual permite buscar las canciones mayores a esta
            dias(String): cantidad de dias introducido por el usuario
    '''
...
def busqueda_repositorio(cursor, opcion, nombre):
    '''
    Función que muestra las canciones buscadas por el usuario ya sea introduciendo el nombre de la canción o del artista deseado
        Parametros:
            cursor: objeto que permite la conexión con SQL Server
            opcion (String): opción por la cual se quiere buscar la cancion, 1 si es por artista, 2 si es por nombre de la canción
            nombre (String):  string que contiene el nombre del artista o de la canción que se quiere buscar.
    '''
...
def top_15(cursor):
    '''
    Función que muestra el top 15 artistas con mas reproducciones dentro del top_10
        Parametros:
            cursor: objeto que permite la conexión con SQL Server
    '''
...
def peak_artista(cursor, artista):   
    '''
    Función que muestra la posición mas alta que obtuvo dentro del top_10
        Parametros:
            cursor: objeto que permite la conexión con SQL Server
            artsta (string): artista del cual se quiere saber cual fue su posición mas alta dentro del top 10.
    '''
...
def promedio_artista(cursor, artista):
    '''
    Función que muestra el promedio de reproducciones en total de todas las canciones de un artista
        Parametros:
            cursor: objeto que permite la conexión con SQL Server
            artista(String): artista del cual se quiere saber el promedio de todas las reproducciones de sus canciones
    '''
...
def main():
    ...
    while opciones:
        ...
        if a == 1:
            ...
            mostrar_reproducciones(cursor, orden)
            mostrar_opciones()
        elif a == 2:
